...
function modify_table(data){
    ...
    for(var i=0;i<data.length;i++){
        ...
        var disponibilidade=document.createElement('td');
        var disponibilidade_checkbox=document.createElement('input');
        disponibilidade_checkbox.setAttribute('type','checkbox');
        disponibilidade_checkbox.setAttribute('name','checkbox'+i);
        disponibilidade_checkbox.setAttribute('value',data[i]['automovel_id']);
        disponibilidade.appendChild(disponibilidade_checkbox);
...
document.getElementById('form-selector').addEventListener('change',function(){table_display()});
//onchange(table_display());
function table_display(){
    if(document.getElementById('form-selector').value=='1'){      
        document.getElementById('search-container').style.display='unset';
    } else{
